...
## Training models
We provide a script in `train.py`, for training with various configurations provided in the lane-detection model. You may use it as a reference to write your own training script.
...
1. **Run Testing Script**:
   ...
   Execute the following command to start testing your video file:
    ...
    ```
    !python test_video.py --video /content/sample_data/sample_video.mp4 --height 480 --width 640 --model /content/lanenet-lane-detection-pytorch/log/best_model.pth --model_type ENet 
    ```

## Results
### 1) Training and Validation Logs
We track the loss over epochs for both the training and validation datasets. The following graphs illustrate the performance of the model across three different loss metrics: Total Loss, Binary Loss, and Instance Loss.

1. Total Loss over Epochs 

    ![Total Loss over Epochs](Lane-Detection/results/TotalLoss_Epochs.png)
